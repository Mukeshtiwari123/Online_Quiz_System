...
public class QuizDaoImpl implements QuizDao {
	...
	public List<Quiz> getAllQuizzes() {
	    Transaction transaction = null;
	    List<Quiz> quizzes = null;  // Declare the quizzes list outside try-catch block
	    try (Session session = HibernateUtil.getSessionFactory().openSession()) {
	        transaction = session.beginTransaction();
	        
	        // Fetch all quizzes along with leaderboardEntries
	        quizzes = session.createQuery(
	            "SELECT DISTINCT q FROM Quiz q LEFT JOIN FETCH q.leaderboardEntries", Quiz.class)
	            .getResultList();
	    ...
	    return quizzes;  // Return the quizzes list
	}
